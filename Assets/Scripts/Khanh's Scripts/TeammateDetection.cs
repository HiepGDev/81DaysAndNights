            ...
            foreach (var targetTag in targetTags)
            {
                ...
                try
                {
                    if (target.CompareTag(targetTag))
                    {
                        ...
                        // quét toàn bộ DefaultRaycastLayers
                        if (Physics.Raycast(eyePosition, directionToTarget.normalized, out RaycastHit hit, distanceToTarget, Physics.DefaultRaycastLayers, QueryTriggerInteraction.Ignore))
                        {
                            // Nếu tia đụng trúng vật cản không phải là mục tiêu và không phải là bản thân Teammate
                            if (hit.transform.root != target.transform.root && hit.transform.root != transform.root)
                            {
                                break; // Bị tường/vật cản chặn, thoát vòng lặp để xét mục tiêu tiếp theo
                            }
                        }
